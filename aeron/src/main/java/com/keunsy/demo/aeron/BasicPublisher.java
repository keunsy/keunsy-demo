...
public class BasicPublisher {
  ...
  //修改发送数据大小
  private static final UnsafeBuffer BUFFER = new UnsafeBuffer(BufferUtil.allocateDirectAligned(25600, 64));
  ...
  public static void main(final String[] args) throws Exception {
    ...
    try (Aeron aeron = Aeron.connect(ctx); Publication publication = aeron.addPublication(CHANNEL, STREAM_ID)) {
      for (long i = 0; i < NUMBER_OF_MESSAGES; i++) {
        final String message = StringUtils.repeat("Hello World! " + i, 1000);
